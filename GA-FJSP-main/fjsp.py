...
DATASET_DIR = r'D:\博士工作内容\博士论文\2023春季学期\1read\model parallelism\代码\Genetic-Algorithm-Flexible-Job-Shop-Scheduling-Problem-master\GA-FJSP-main'
import pandas as pd
import os
import random
import numpy as np
...
def fix_individual(individual):
  ...
  for i in individual_df.index:
    ...
    if(operation - prev_lot_op != 1):
      ...
      fixed_df = (pd.concat([individual_df, lot_req_op], ignore_index=True))
    ...
    fixed_df = pd.concat([individual_df, df], ignore_index=True)
